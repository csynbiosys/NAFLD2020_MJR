...
```{r}
setwd("~/Desktop/Google Drive/Documents/PhD/First year/Differential expression/SteatoSITE")
library(edgeR)
library(pheatmap)
library(PCAtools)
library(ggplot2)
...
```{r}
feature_counts <- read.csv("./featurecounts+coordsorted+gene_lib.csv", stringsAsFactors = FALSE, row.names = 1)
...
The pathological metadata csv file contains the following columns:
  ...
  *Lobular_inflammation_score: from 0 to 3. Overall assessment of all inﬂammatory foci.
...
```{r}
pathological_data <- read.csv("./path_metadata.csv", header = TRUE, stringsAsFactors = FALSE)
...
There are `nrow(pathological_data)` samples in the pathological metadata, and `ncol(feature_counts)` samples in the counts matrix. This is because the pathological metadata only contains the NAFLD-NASH spectrum, hence `ncol(feature_counts)-nrow(pathological_metadata)` samples, are controls. It is important to mention that the pathological metadata does contains some control samples, but they slipped in by mistake. Another thing to take into account, is that there are samples in the pathological metadata and counts matrix that do not come from the liver. For this reason, these samples are eliminated.

```{r}
not_in_pathological <- names(feature_counts)[!names(feature_counts) %in% pathological_data$sample_id]
pathological_data <- add_row(pathological_data, sample_id = c(not_in_pathological), hepatocyte_ballooning = c(rep(0, times = length(not_in_pathological))), kleiner_brunt_steatosis = rep(0, times = length(not_in_pathological)), lobular_inflammation = rep(0, times = length(not_in_pathological)), modified_ishak_score = rep(0, times = length(not_in_pathological)), kleiner_fibrosis = rep("0", times = length(not_in_pathological)), kleiner_fibrosis_combined = rep("0", times = length(not_in_pathological))) #As they are control samples, the scores will always be 0.
c("1122", "1135", "1202", "2205", "2279") %in% names(feature_counts) ##We look for the non-liver samples: 1122, 1202 and 2279 are present.
```
...
First, we are going to normalised the samples and create a MDS plot to see if the technical replicates (those samples that have the same sample_id) cluster together or not. 
...
```{r}
table(dgList$samples %>% pull(kleiner_fibrosis))
table(dgList$samples %>% pull(hepatocyte_ballooning))
table(dgList$samples %>% pull(lobular_inflammation))
table(dgList$samples %>% pull(kleiner_brunt_steatosis))
table(dgList$samples %>% pull(modified_ishak_score))
```
...
for (i in names(PCA$metadata[5:ncol(PCA$metadata)])){
  p <- biplot(pcaobj = PCA, x = "PC1", y = "PC2", colby = i, title= "Biplot of the logCPM", legendPosition = "bottom", lab = NULL)
  print(p)
  p<- biplot(pcaobj = PCA, x = "PC2", y = "PC3", colby = i, title= "Biplot of the logCPM", legendPosition = "bottom", lab = NULL)
  print(p)
}
```
...
gene_symbols <- read.csv("./Gene_symbols_ensembl.csv", row.names = 1)
