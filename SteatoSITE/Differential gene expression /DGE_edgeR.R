...
#Differential gene expression using edgeR. It chooses those genes according to chosen values for FDR < fc and |FC| >= fc.The output is a list that contains the main objects of the analysis, as well as the genes that meet the criteria.
dea_edgeR <- function(dgList, fc, fdr, group_var = "group", blocking_vars = c(), contrast_name, print =TRUE){
  fc_threshold <- fc
  fdr_threshold <- fdr
