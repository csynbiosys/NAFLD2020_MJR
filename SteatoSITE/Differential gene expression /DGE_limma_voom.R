...
#Differential gene expression with limma+voom. It chooses those genes with FDR < 0.05 and |fold-change| >= 2. The output returns a list of objects obtained during the analysis, including those genes that meet the criteria.
dea_limma <- function(dgList, group_var = "group", blocking_vars = c(), contrast_name, print =TRUE){
  fc_threshold <- 2
  fdr_threshold <- 0.05
